...
// score each project by how many of the selected tags it has (and what
// percentage of the selected tags that represents), then sort so the
// projects matching the most float to the top.
export function rankProjects(projects: Project[], selectedTags: string[]) {
  ...
  return projects
    .map((project) => {
      ...
      return { ...project, score: matchCount, percentage };
    })
